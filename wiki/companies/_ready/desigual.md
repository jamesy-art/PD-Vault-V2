---
company_uid: 8c96055f-efda-46d1-8046-21e658afe2db
schema_version: 1
name: Desigual
slug: desigual
publication_status: published
origin: vault
company_type: brand
categories:
  - accessories
  - fashion
  - jewellery
  - kids
markets:
  - womenswear
  - menswear
  - kidswear
products:
  - footwear
  - apparel
  - accessories
  - bags
  - jewellery
  - swimwear
  - kidswear
hq_country_code: ES
location: Spain
founded_year: 1984
employee_count_text: 2,600
revenue_text: €560M
website_url: https://www.desigual.com
instagram_url: https://www.instagram.com/Desigual
linkedin_url: https://www.linkedin.com/company/desigual/
facebook_url: https://www.facebook.com/Desigual
tiktok_url: https://www.tiktok.com/@desigual
youtube_url: https://www.youtube.com/user/webdesigual
type_slugs:
  - ethnic-pattern
  - floral-pattern
  - bohemian-pattern
  - patchwork-pattern
  - abstract-pattern
images:
  logo:
    path: ../../assets/companies/desigual/profile/desigual-logo.png
    alt: Desigual logo
    width: 742
    height: 742
  gallery:
    - title: Clothing
      alt: Desigual clothing collection
      path: ../../assets/companies/desigual/gallery/desigual-clothing.jpg
      width: 1080
      height: 1619
      source:
        title: null
        page_url: null
      affiliate:
        enabled: false
        url: null
    - title: Dresses
      alt: Desigual dresses collection
      path: ../../assets/companies/desigual/gallery/desigual-dresses.jpg
      width: 1080
      height: 1619
      source:
        title: Dresses
        page_url: https://www.desigual.com/en_TT/women/clothing/dresses
      affiliate:
        enabled: false
        url: null
updated: '2026-08-02'
enrichment_status: enriched
marketing_order: 201
---

# Desigual

## Profile Image

![Profile](../../assets/companies/desigual/profile/desigual-logo.png)

## Gallery Images

| Title | Preview |
|-------|---------|
| Clothing | ![](../../assets/companies/desigual/gallery/desigual-clothing.jpg) |
| Dresses | ![](../../assets/companies/desigual/gallery/desigual-dresses.jpg) |

## Overview

Desigual (Spanish/Catalan: "unequal, uneven") was founded in Barcelona by Thomas Meyer in 1984, with the name itself signalling the brand's anti-normative aesthetic position. The company operates in 109 countries, with 393 mono-brand stores and a centralised logistics structure based in Viladecans and Gavà (Spain), New Jersey, and Hong Kong. As of 2022, Desigual employs more than 2,600 people across all markets, with 547 based at the Barcelona headquarters. The company reported revenues of €560 million in 2011 and achieved 60% annual growth between 2002 and 2009.

Desigual's design language is maximalist, pattern-saturated, and culturally eclectic. Inspiration draws consistently from global folk textile traditions, mixed-media garment construction, and expressive colour contrast. The founding patchwork logic - combining disparate fabric panels, prints, and textures into a unified garment - runs through the brand's product range. Collections reference ethnic fabric traditions including woven, embroidered, and printed textiles from multiple global origins.

A 2019 product repositioning shifted the range towards more urban, versatile constructions while retaining the pattern and colour density that defines the brand. The art-gallery retail concept - individual garments as statement pieces - aligns the brand with wearable-art positioning.

## History

The brand traces its identity to a single garment: a jacket constructed by founder Thomas Meyer in 1983 from scraps of second-hand denim, later known as the "Iconic Jacket." This patchwork, bricolage aesthetic - materials stitched together from disparate sources - became the brand's founding design logic. The Barcelona flagship was established in 1986 in Ibiza harbour. The company grew rapidly through the 2000s, reaching €440 million in revenue by 2010.

A significant brand identity overhaul in 2019 introduced a reversed-text logo, a product range repositioned as more urban and versatile, and a new retail concept modelled on art gallery formats - fewer garments per store, increased focus on the individual piece. The company's Harajuku (Tokyo) store offered garment customisation workshops alongside the product range.

In 2021, Desigual became the first international fashion brand in Spain to implement a four-day working week, approved by an 86% employee vote.

## Products

- **Womenswear:** Core category; print-heavy dresses, tops, jackets, and knitwear; two collections annually (spring-summer, autumn-winter)
- **Menswear:** Graphic-led casual wear
- **Childrenswear:** Pattern and character-led ranges
- **Accessories:** Bags, footwear, jewellery
- **Denim:** The brand has a recurring denim motif across patchwork and mixed-fabric constructions referencing the founding jacket
- **Sustainable lines:** "Love the World" collection (2021) featuring organic and recycled fibres

## Services

Desigual distributes through ten sales channels: physical mono-brand stores, multi-brand wholesale, e-commerce (desigual.com, active in 150+ markets, backed by Global-e for cross-border), e-tailers, and flash-sale platforms. RFID technology is deployed across all physical stores. The "Ship From Store" fulfilment model is operational. Online sales grew 50% on average from May 2020, with peaks of up to 70%.

Desigual is a relevant context for surface pattern designers working in womenswear print, maximalist apparel, ethnic-inspired textile design, mixed-media fashion construction, and print-heavy accessories. The brand's design brief consistently requires pattern complexity across multiple garment categories simultaneously.

## Markets

Desigual occupies a mid-market position in European fashion, priced above mass-market fast fashion but below premium. Its key differentiator is surface design density: the brand is known for garments with high print, colour, and embellishment complexity at accessible price points. The core customer base is adult women seeking expressive, colour-led fashion.

The brand's international footprint spans 109 countries with a strong European base and significant Asian presence (Japan, China, India documented). Distribution includes mono-brand stores, online, e-tailers, and flash-sale channels. Desigual joined the Textile Exchange and has sustainability partnerships with the Sustainable Apparel Coalition (SAC), Better Cotton Initiative (BCI), the Fashion Pact, Amfori, and Sedex.

## Pattern and Surface Design Relevance

Desigual's commercial identity is inseparable from surface pattern and textile design. The brand uses ethnic-derived textile references, dense floral and geometric motifs, mixed-print layering, patchwork construction, and embellishment as primary design elements. Collection names across its history - "Rainbow," "Magic Stories," "Handmade," "El Love," "Luxury Feelings" - consistently signal pattern and textile richness rather than silhouette as the selling proposition.

Documented design collaborations include work with Christian Lacroix (French fashion designer, from 2011), Spanish artists Miranda Makaroff and Okuda San Miguel, designer María Escoté, and the Cirque du Soleil partnership (2011, 60-item clothing and accessories collection). The Disney collaboration drew on the founding denim jacket aesthetic - mixed fabric, embellishment, colour complexity - rather than character licensing. These partnerships have primarily involved surface design and print direction as the design brief.

In 2014, the brand received the bronze Laus Award for its 2019 brand identity redesign. The "Iconic Jacket" remains a recurring reference point for mixed-textile, pattern-layered outerwear design.

## Official Resources

- [About](https://www.desigual.com/en_TT/corporate/about-us)
